    ...
    useEffect(() => {
        const fetchStudent = async () => {
            const response = await API.post(`/${ModelType.student}`, {
                query: [{ key: 'id', value: Number(id), opt: 'eq' }],
                relation_model: true
            });
            ...
            if (student_obj) {
                ...
                dispatch(setUserAndStudentId({ user_id: student_obj.teacher_id, student_id: student_obj.id }));
            }
        };
        fetchStudent().catch((err) => console.error('Failed to fetch student', err));
    }, [id, dispatch]);
